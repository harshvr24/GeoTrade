...
import json  # Only used in NewsDataManager
...
# Configuration constants (replacing magic values)
class SignalConfig:
    """Configuration constants for trading signal generation."""
    MEDIUM_CONFIDENCE = 0.65
    HIGH_CONFIDENCE = 0.75
    MEDIUM_SEVERITY_BUY_CONFIDENCE = 0.62
    HIGH_SEVERITY_BUY_CONFIDENCE = 0.72
    MIN_MARKET_IMPACT = 1.5
...
class NewsIntegrator:
    ...
    def generate_trading_signal(event: dict[str, Any]) -> dict[str, Any] | None:
        ...
        # Use default of 0.0 to avoid None comparison issues
        market_impact = event.get("market_impact") or 0.0
        if market_impact < SignalConfig.MIN_MARKET_IMPACT:
        ...
        if any(word in headline_lower for word in ["risk", "escalat", "tension", "sanction"]):
            action = "SELL"
            confidence = SignalConfig.MEDIUM_CONFIDENCE if severity == "medium" else SignalConfig.HIGH_CONFIDENCE
        else:
            action = "BUY"
            confidence = SignalConfig.MEDIUM_SEVERITY_BUY_CONFIDENCE if severity == "medium" else SignalConfig.HIGH_SEVERITY_BUY_CONFIDENCE
