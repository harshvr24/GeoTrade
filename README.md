...
Backend serves:
- REST v1 under `/api/...`
- REST v2 under `/api/v2/...`
- WebSockets: `/ws/stream` (v1 snapshot), `/ws/stream/v2` (enriched snapshot)

Optional env flags (all default to safe no-op):
- `REDIS_URL` to enable Redis cache
- `KAFKA_BOOTSTRAP` to enable Kafka producer
- `LLM_PROVIDER` to switch from heuristic to real LLM (future OpenAI/Ollama hook)
...
VITE_WS_URL=ws://localhost:8000/ws/stream/v2   # optional; falls back automatically
```

### 3) Ingestion worker (optional, RSS ingest)
Runs a lightweight fetch/dedupe/enrich and caches results (in-memory or Redis if configured):
```bash
cd backend
python -m app.ingestion.worker
```
Schedule it separately (cron/systemd) for continuous ingestion.

### Build / Production
- Backend: `uvicorn app.main:app --host 0.0.0.0 --port 8000` (or any ASGI server). Vercel serverless adapter provided at `api/index.py`.
- Frontend: `npm run build` then serve `frontend/dist` with any static host.

## Run Order (dev)
1) Start backend: `uvicorn app.main:app --reload --port 8000`
2) (Optional) Run ingestion worker to load fresh RSS events.
3) Start frontend: `npm run dev`

If `npm run build` fails on Windows due to home directory permissions, set a local npm cache:
```bash
